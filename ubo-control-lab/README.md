...
The candidate review displays six source-backed facts from one Artifact and stays explicitly non-operative. The fixture-only Compliance helper then applies exact source-occurrence identity decisions and claim decisions through Decision Application v3. Snapshot B retains Snapshot A and four economic graph relationships, while two officer-role facts remain non-control metadata.

The frozen Evidence fixture establishes no signed/as-at declaration, signer, capacity, attestation scope or attestation locator. All relationship DTOs therefore retain their extracted `UNKNOWN` temporal state. The Artifact `capturedAt` value is ingestion metadata, not a source-effective date. The source percentages remain inspectable—Mitchell's nominal path is 75% × 100% and Lee's is 25% × 100%—but current qualification is `INDETERMINATE` and the deterministic calculations remain `UNRESOLVED` for `UNKNOWN_TEMPORAL_STATE`. R08 Evidence sufficiency stays separate and unresolved.
