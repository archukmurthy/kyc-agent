...
function provisionalUboLabel(state, method) {
  if (method !== "EFFECTIVE_INTEREST") return state;
  if (state === "SATISFIED") return "Provisional UBO · qualifies";
  if (state === "NOT_SATISFIED") return "Provisional UBO · does not qualify";
  if (state === "INDETERMINATE") return "Provisional UBO · cannot determine";
  return "Provisional UBO · review required";
}

export default function ChartAnalysisPanel({ analysis, method, onMethodChange, legacyProjection = null, sourceProjection = null, coverage = null, companyName = "the customer under review" }) {
  ...
  const incomplete = coverage?.state === "REVIEW_REQUIRED";
  return <section className="ubo-customer-chart-analysis" aria-label="Uploaded chart analysis">
    <div className={`ubo-customer-analysis-status ${incomplete ? "warning" : ""}`} role={incomplete ? "alert" : undefined}><div><small>Chart analysis status</small><strong>{incomplete ? "Incomplete ownership map" : "Provisional chart assessment"}</strong></div><p>{incomplete ? <>The extracted relationship chain does not reach {companyName}. The visible source facts remain available, but this result is not the complete chart.</> : <>This is a separate assessment of the uploaded chart. Filters change the presentation only; effective ownership uses the source-backed ownership chain. The chart has not been independently verified as current.</>}</p></div>
    ...
    <section className="ubo-customer-card ubo-customer-method-card"><header><div><small>Recorded engine output</small><h2>Calculation view</h2></div><button className="ubo-customer-collapse-button" type="button" aria-expanded={calculationExpanded} aria-controls="ubo-customer-calculation-content" onClick={() => setCalculationExpanded((value) => !value)}>{calculationExpanded ? "Collapse" : "Expand"}</button></header>
      {calculationExpanded && <div id="ubo-customer-calculation-content"><fieldset><legend>Calculation method</legend>{CALCULATION_METHODS.map((option) => <label key={option.value}><input type="radio" name="chart-calculation-method" value={option.value} checked={method === option.value} onChange={() => onMethodChange(option.value)} />{option.label}</label>)}</fieldset>
      ...
      {people.length ? <div className="ubo-customer-calculations">{people.map((person) => <article key={person.personEntityId}>
        <header><div><small>{person.personName}</small><h3>{person.selectedResultLabel}</h3></div><span>{provisionalUboLabel(person.selectedState, method)}</span></header>
      ...
      </article>)}</div> : <div className="ubo-customer-calculation-empty"><p className="ubo-customer-muted"><strong>No effective-ownership result has been recorded yet.</strong> The chart facts did not produce an operative natural-person qualification record. This is not a negative UBO conclusion.</p>{ownershipSteps.length > 0 && <><p>The following ownership steps remain available from the chart, but they have not been combined into an effective ownership result:</p><ul>{ownershipSteps.map((step) => <li key={step.relationshipId}><strong>{step.from}</strong> → {step.to}: {formatAssertionMeasurement(step.measurement)}</li>)}</ul></>}</div>}</div>}
    </section>
  </section>;
}

export { graphForChartPresentation, projectGraph, provisionalUboLabel, sourceOwnershipSteps };
