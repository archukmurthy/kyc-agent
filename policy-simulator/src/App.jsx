  ...
  return (
    <form onSubmit={(event) => { event.preventDefault(); if (valid) onGenerate(); }} className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
      ...
      <Section number="2" title="Customer types in scope" description="Select all customer types to include.">
        <CheckboxGroup label="Customer types" options={CUSTOMER_TYPES} selected={form.customerTypes} onToggle={(value) => toggle('customerTypes', value)} />
      </Section>
      ...
      <Section number="4" title="Products & services" description="Select all products and services to include.">
        <CheckboxGroup label="Products and services" options={PRODUCTS} selected={form.products} onToggle={(value) => toggle('products', value)} />
      </Section>
      ...
      <Section number="6" title="Verification vendors" description="Select all verification sources to reference.">
        <CheckboxGroup label="Verification vendors" options={VENDORS} selected={form.vendors} onToggle={(value) => toggle('vendors', value)} />
...
function OutputView({ policyHtml, onStartAgain }) {
  ...
  return (
    <main className="min-h-screen bg-slate-50 px-4 py-10 sm:px-6 sm:py-14">
      <div className={view === 'output' ? 'mx-auto max-w-5xl' : 'mx-auto max-w-3xl'}>
        ...
        {view === 'output' && <OutputView policyHtml={policyHtml} onStartAgain={startAgain} />}
      </div>
    </main>
  );
}
