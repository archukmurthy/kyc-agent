 ...
 * Demo Document Requirements Service (DRS)
 * 
...
/**
 * Derives the Demo onboarding/contracting entity from the client's country
 * of incorporation. This is the country whose licence and rule set applies.
 * 
 * Rule: if Demo is licensed in the incorp country → onboard there.
...
function uboThresholdText(country){
  ...
  if (country === 'New Zealand') return 'More than 25% ownership/control threshold; exactly 25% is not caught by the ownership test alone, so also capture the person where they otherwise exercise effective control (per NZ AML/CFT Act 2009 and DIA beneficial-ownership guidance).';
  if (country === 'Indonesia') return '25% ownership/control threshold per Indonesia Presidential Regulation No. 13 of 2018 on Beneficial Ownership; trace through the ownership chain to the natural-person beneficial owner, continuing where effective control, opacity or risk assessment requires it.';
  if (country === 'Lithuania') return '25% ownership/control baseline per EU AMLD (Directive (EU) 2015/849 as amended) and the Lithuanian Law on the Prevention of Money Laundering and Terrorist Financing; apply a lower threshold on a risk-based / enhanced due diligence basis where required.';
  return '25% ownership/control baseline consistent with FATF Recommendation 10 and the EU AMLD standard; apply a lower threshold on a risk-based / enhanced due diligence basis where required.';
}
...
function notFeasible(req, entityType, sector, incorp){
  ...
  if (req === 'Ownership / control'){
    ...
    return 'Escalate to Compliance if ownership cannot be traced to a reliable UBO or permitted fallback.';
  }
  if (req === 'UBO ID') return uboFallbackText(country) + ' Collect senior managing official ID where the fallback applies (per FATF Recommendation 10 / EU AMLD Art. 3(6)), and record the rationale.';
...
function localEquivalent(country, req, entityType, sector, onboarding){
  ...
  if (individualMode){
    const m = {
      'Customer application form':'Demo customer application form, signed by the individual customer or authorised representative',
  ...
  if (entityType === 'Trust'){
    const m = {
      'Customer application form':'Demo customer application form, signed by trustee or authorised representative',
      'Purpose of account / relationship':'Purpose of account declaration within the customer application form / onboarding questionnaire',
  ...
  if (entityType === 'Fund'){
    const m = {
      'Customer application form':'Demo customer application form, signed by GP, manager, trustee, responsible entity or authorised representative',
  ...
  if (entityType === 'Partnership/LLP'){
    const m = {
      'Customer application form':'Demo customer application form, signed by partner/member or authorised representative',
      'Purpose of account / relationship':'Purpose of account declaration within the customer application form / onboarding questionnaire',
  ...
  const m = {
    'Customer application form':'Demo customer application form, signed by an authorised director, officer, trustee, partner, authorised signatory or person with delegated authority',
...
function applyIndonesiaOverlay(rows, onboarding, incorp, entityType, sector){
  ...
  rows.forEach(r => {
    ...
    if (r.requirement === 'Ownership / control') {
      r.localEquivalent = 'Shareholder register + ownership chart tracing through the ownership chain to the natural-person beneficial owner where a 25% ownership/control interest exists';
      r.analystStep = 'Apply the Indonesia 25% ownership/control threshold (Presidential Regulation No. 13 of 2018 on Beneficial Ownership). Trace ownership through the chain to the natural-person beneficial owner where a 25% ownership/control interest exists, and continue further if control, opacity, sanctions/PEP exposure or risk assessment requires it.';
...
function applyUkSignatoryAuthorityOverlay(rows, onboarding, incorp){
  ...
  // The UK requirement (Money Laundering Regulations 2017, risk-based CDD) is
  // authority evidence on a risk-based basis, not blanket residential address collection.
...
function buildRows(onboarding, incorp, entityType, sector){
  ...
  rows = applyIndonesiaOverlay(rows, onboarding, incorp, entityType, sector);
 ...
 * @param {string} [params.onboardingCountry]   - Demo contracting entity country (derived if omitted)
