...
UK Corporate 1.6-RC is an immutable schema-1.3 review-policy artifact with canonical hash `sha256:6f4235ca32b961868f294b862810d101516a35a5ce8fe8a031ec2d2166e6e969`. It is selected only by the isolated successor-review Lab through `ubo-control/review`; it is not selected by the stable Decision Application v1/v2 or host onboarding. Readiness assessment returns `REVIEW_ONLY` in LAB and `BLOCKED` in PRODUCTION because it is not approved/effective and required sign-offs remain unsigned. The readiness operation itself does not execute doctrine.
