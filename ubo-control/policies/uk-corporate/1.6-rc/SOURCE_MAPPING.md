...
| Layer completeness | File 01 §5; File 02 §6 | Control Room doctrine | A-13 where joint facts matter | Internal review-mode assessment implemented; production unauthorized and unwired | Wave 6 complete internally; runtime later | `ubo-layer-closure-v1` evaluates one direct layer/dimension with exact residual endpoints and separate statutory/firm results |
| Percentage evidence states | File 01 §5.3; File 02 §7 | Control Room evidence doctrine | A-03 | Internal classification implemented; operational sufficiency not implemented | Wave 6 classification complete; policy sufficiency later | Declaration, band corroboration and capable exact verification remain distinct; only declaration+band records the A-03 gate |
| Decision-sensitive precision | File 01 §5.4; File 02 §6 | Control Room doctrine | None for the diagnostic itself | Internal review-mode diagnostic implemented; unwired | Wave 6 complete internally; needs/questions later | Exactness is required only when it can change threshold/layer closure; non-percentage blockers suppress false requests |
