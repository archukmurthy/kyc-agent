...
class SemanticExtractionProvider { async extract() { throw new Error("SemanticExtractionProvider.extract must be implemented"); } }
...
class FixtureSemanticProvider extends SemanticExtractionProvider {
  ...
  async extract({ artifact, artifactInputs = [], content, decodedText, requestedConcepts = [] }) {
    ...
    return { facts: facts.map((fact) => ({ ...fact, valueFound: true, semanticRole: "business_fact", sampled: false, supportingArtifactIds: inputIds.slice(0, 1) })), requestedConceptOutcomes: requestedConcepts.map((request) => ({ concept: request.concept, status: facts.some((fact) => fact.requested && fact.concept === request.concept) ? "found" : "not_found" })), completeness: { state: "complete", limitations: [] }, support: { state: facts.length ? "supported" : "not_supported", signals: { readability: "clear", extractionMethod: "fixture_ai", literalFieldMatchRequired: false } } };
  }
}
...
class AnthropicSemanticProvider extends SemanticExtractionProvider {
  constructor({ apiKey, model, fetchImpl = global.fetch, timeoutMs = DEFAULT_PROVIDER_TIMEOUT_MS, maxOutputTokens = DEFAULT_MAX_OUTPUT_TOKENS, instructionReference = "evidence-a3-live-v5-multi-artifact" } = {}) {
  ...
  async extract({ artifact, artifactInputs = [], decodedText, requestedConcepts = [], context = {}, sourceMetadata = {} }) {
    ...
    const inputSections = inputs.map((input) => [
      `BEGIN VERIFIED ARTIFACT ${JSON.stringify({ id: input.artifact?.id, order: input.order ?? null, mediaType: input.artifact?.mediaType, representationType: input.artifact?.representationType, capturedAt: input.artifact?.capturedAt, source: input.artifact?.sourceLocator })}`,
      String(input.decodedText || ""),
      `END VERIFIED ARTIFACT ${input.artifact?.id || "unknown"}`,
    ].join("\n"));
    const prompt = [
      ...
      "Each facts item must contain concept, value, raw, requested, value_found, semantic_role, sampled, supporting_artifact_ids, semanticAmbiguity, and ambiguous. semantic_role must be business_fact for a persistable semantic fact.",
      ...
      `Evidence input manifest: ${JSON.stringify(inputs.map((input) => ({ id: input.artifact?.id, order: input.order ?? null, mediaType: input.artifact?.mediaType, representationType: input.artifact?.representationType })))}`,
      ...
      "Verified Artifact inputs follow with explicit boundaries:", ...inputSections,
    ].join("\n\n");
    const requestBody = JSON.stringify({ model: this.model, max_tokens: this.maxOutputTokens, messages: [{ role: "user", content: prompt }] });
    let response;
    try {
