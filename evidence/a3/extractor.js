...
function factFromProvider(item, context) {
  ...
  const signals = { readability: item.ambiguous ? "degraded" : "clear", grounding: "direct", extractionMethod: "ai_semantic", semanticMappingAmbiguity: !!item.semanticAmbiguity, multiplePlausibleValues: !!item.ambiguous, providerOutputUnicodeNormalized: concept.changed || factValue.changed || raw.changed, supportingArtifactCount: supportingArtifactIds.length };
  return { id: context.idFor(item), extractionRunId: context.runId, artifactId: supportingArtifactIds[0], supportingArtifactIds, informationNeedId: item.requested ? item.informationNeedId || null : null, schemaFieldId: item.requested ? item.schemaFieldId || null : null, semanticConceptId: concept.value, requestStatus: item.requested ? "requested" : "discovered", groundingType: "direct", factValue: factValue.value, rawRepresentation: raw.value, supportState: supportState(signals), supportSignals: signals, sourcePolicyContext: context.sourcePolicyContext || {}, createdAt: context.createdAt };
}
...
function isAbsencePlaceholder(value) {
...
async function runSemanticExtraction(provider, request) {
  ...
  const noValueFacts = providerFacts.filter((item) => !item || !item.concept || item.valueFound !== true || item.value === null || item.value === undefined || isAbsencePlaceholder(item.value));
  ...
  const completeness = { state: result.completeness?.state === "complete" && inputCompleteness.state === "complete" && !sampledFacts.length && !invalidSupportFacts.length ? "complete" : "incomplete", limitations: [...new Set(limitations)], sourceRecordCount: result.completeness?.sourceRecordCount ?? null, representedRecordCount: result.completeness?.representedRecordCount ?? null };
