...
test("access, storage, unsupported media, provider and empty-result failures preserve A2", async (t) => {
  ...
  await t.test("unsupported image", async () => { const repository = new TestRepository(artifact({ mediaType: "image/png" })); await assert.rejects(() => service({ repository }).interpret({ artifactId: "artifact-real", tenantId: "nium" }), (e) => e.code === "unsupported_media_type"); assert.equal(repository.appends[0].facts.length, 0); });
